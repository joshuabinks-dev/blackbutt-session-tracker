...
window.__newSessionFromPreset = async function(){
  if (state.activeSessionId){
    toast("End or resume the active session first");
    return;
  }
  ...
  const s = {
    ...
    name: preset?.name || "New session",
    location: preset?.location || "",
    startedAt: nowISO(),
    endedAt: null,
    blocks: structuredClone(preset?.blocks || []),
    athletesSnapshot: structuredClone(state.athletes.length ? state.athletes : []),
    results: {}, // key: athleteId|blockIndex|repIndex -> seconds
    live: null
  };
  // If no saved athletes, create a couple of placeholders (inactive)
  if (s.athletesSnapshot.length===0){
    s.athletesSnapshot = [
      {id:uid(), first:"First", last:"Athlete", group:"A", active:false},
      {id:uid(), first:"Second", last:"Athlete", group:"B", active:false},
    ];
  }
