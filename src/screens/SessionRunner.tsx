      ...
      <div className="card">
        ...
        <div className="grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
          <div>
            ...
            <input defaultValue={session.name} onChange={(e) => api.setSessionName(e.target.value)} />
          </div>
          <div>
            <label>Location</label>
            <select value={session.location} onChange={(e) => api.setSessionLocation(e.target.value)}>
              <option value="">— Select —</option>
              {locations.slice().sort((a,b)=>a.localeCompare(b)).map(l => <option key={l} value={l}>{l}</option>)}
              <option value="__custom__">Custom…</option>
            </select>
            {session.location === "__custom__" && (
              <div style={{ marginTop: 8 }}>
                <input placeholder="Enter custom location" onChange={(e)=>api.setSessionLocation(e.target.value)} />
              </div>
            )}
          </div>
        </div>
