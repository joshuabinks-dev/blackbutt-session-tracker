...
export function SessionScreen() {
  ...
  if (!session) {
    return (
      <div className="card">
        <h2>Session</h2>
        <div className="muted">No active session. Start one from Home → Templates.</div>
      </div>
    );
  }
    ...
    <div className="grid two">
      <div className="card">
        <div className="hrow">
          <h2>Session setup</h2>
          <div className="row">
            <button className={"btn " + (session.allInMode ? "primary" : "")} onClick={() => api.toggleAllIn()}>
              All-in: {session.allInMode ? "ON" : "OFF"}
            </button>
            <button className="btn danger" onClick={() => api.endSession()}>End</button>
          </div>
        </div>
        ...
        <div className="grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
          ...
          <div>
            ...
            <select value={locSelect} disabled={!!session.endedAtISO} onChange={(e) => { const v = e.target.value; setLocSelect(v); if (v !== "__custom__") { api.setSessionLocation(v); } }}>
            ...
            {locSelect === "__custom__" && (
              <div style={{ marginTop: 8 }}>
                <input value={customLoc} disabled={!!session.endedAtISO} placeholder="Enter custom location" onChange={(e)=>{ setCustomLoc(e.target.value); api.setSessionLocation(e.target.value); }} />
              </div>
            )}
          </div>
        </div>
