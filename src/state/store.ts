...
export const useAppStore = create<AppState>((set, get) => {
  ...
  return {
    ...
    captureAthlete: (sessionId, groupId, athleteId) => {
      ...
      // Manual completion: if all active athletes captured, advance to READY and next rep index
      ...
      if (block.timingMode === 'manual') {
        ...
        if (done) {
          nextGs = advanceAfterWorkCompletion(state, session, groupId, 'autoBoundary')
        }
      }
    ...
    autoBoundary: (sessionId, groupId) => {
      ...
      const nextGs = advanceToNextItemOrRep(state, template, session, groupId, 'autoBoundary')
