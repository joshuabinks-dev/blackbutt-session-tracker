...
export function GroupCard(props: { session: LiveSession; groupId: GroupId; nowMs: number }) {
  ...
  const isComplete = gs.status === "complete";
  ...
  let roster = session.roster.filter(a => a.active && a.groupId === groupId);
  if (gs?.sortOrderAthleteIds && gs.sortOrderAthleteIds.length) {
    const rank = new Map(gs.sortOrderAthleteIds.map((id, i) => [id, i] as const));
    roster = [...roster].sort((a, b) => (rank.get(a.id) ?? 1e9) - (rank.get(b.id) ?? 1e9));
  }
    ...
    <div className="card">
      ...
      <div className="timerBox">
        <div>
          ...
          {isComplete && <div className="badgeOk">Complete</div>}
          {!isComplete && !restInfo && gs.status === "idle" && <div className="muted small">Ready</div>}
        </div>
        <div className="row" style={{ justifyContent:"flex-end" }}>
          {isComplete && <button className="btn" onClick={() => api.reopenGroup(groupId)}>Re-open</button>}
          <button className="btn primary" disabled={!canStart} onClick={() => api.startWork(groupId)}>
            {isJoiner ? "Start / Resume" : "Start rep"}
          </button>
          {session.fastCaptureMode && gs.lastCapture && gs.status === "runningWork" && (
            <button className="btn" onClick={() => api.undoLastCapture(groupId)}>Undo last</button>
          )}
          <button className="btn" disabled={!item || isComplete} onClick={() => api.next(groupId)}>Next</button>
        </div>
      </div>
          ...
          <div className="athGrid">
            {roster.map(a => {
              ...
              return (
                ...
                >
                  {session.fastCaptureMode ? (
                    <>
                      {`${a.firstName} ${a.lastName}`.trim()}
                      {done && <span className="small badgeOk">Captured</span>}
                    </>
                  ) : (
                    <>
                      {`${a.firstName} ${a.lastName}`.trim()}
